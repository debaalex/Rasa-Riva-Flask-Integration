...
def execDockCom(ssh, command, idDock):
    ...
    print("output =", output)
    rtfx = output[len(output)].split(":")[1]
    audiolen=output[len(output) - 1].split(":")[1]
    runtime = output[len(output) - 2].split(":")[1]
...
def runTranscript(path):
    ...
    idD = getDockerId(c)
    command = "riva_streaming_asr_client --audio_file /riva/{} --output_filename=/riva/resp.txt".format(basename)
    ...
    print(end - start)
    timer = end - start
    data = [transcript, timer, exec[0], exec[1], exec[2],]
    print(data)
    return data
